...
//            "{\"type\":\"Feature\",\"properties\":{\"mag\":9,\"place\":\"Pacific-Antarctic Ridge\",\"time\":1451986454620,\"updated\":1459202978040,\"tz\":-540,\"url\":\"http://earthquake.usgs.gov/earthquakes/eventpage/us10004bgk\",\"detail\":\"http://earthquake.usgs.gov/fdsnws/event/1/query?eventid=us10004bgk&format=geojson\",\"felt\":0,\"cdi\":1,\"mmi\":0,\"alert\":\"green\",\"status\":\"reviewed\",\"tsunami\":0,\"sig\":554,\"net\":\"us\",\"code\":\"10004bgk\",\"ids\":\",us10004bgk,gcmt20160105093415,\",\"sources\":\",us,gcmt,\",\"types\":\",cap,dyfi,geoserve,losspager,moment-tensor,nearby-cities,origin,phase-data,shakemap,\",\"nst\":null,\"dmin\":30.75,\"rms\":0.67,\"gap\":71,\"magType\":\"mww\",\"type\":\"earthquake\",\"title\":\"M 6.0 - Pacific-Antarctic Ridge\"},\"geometry\":{\"type\":\"Point\",\"coordinates\":[-136.2603,-54.2906,10]},\"id\":\"us10004bgk\"}],\"bbox\":[-153.4051,-54.2906,10,158.5463,59.6363,582.56]}";
    ...
    /**
     * Query the USGS dataset and return an {@link Earthquake} ArrayList object
     * to represent 10 recent list of earthquakes
     */
    public static List<Earthquake> fetchEarthquakequakeDataList(String requestURL) {
        Log.e(LOG_TAG, "Test: QueryUtils fetchEarthquakequakeDataList() called.");

        // Create URL object
        URL url = createURL(requestURL);

        // Perform HTTP request to the URL and receive a JSON response back
        String earthquakeListJson = null;
        try {
            earthquakeListJson = makeHttpRequest(url);
        } catch (IOException e) {
            Log.e(LOG_TAG, "Error closing the input stream " + e);
        }

        // Extract all earthquakes from the json response and store it in a list
        List<Earthquake> earthquakeList = extractEarthquakesFromJson(earthquakeListJson);
    ...
    private static String makeHttpRequest(URL url) throws IOException {
        ...
        try {
            ...
            Log.e(LOG_TAG, "Error retrieving the JSON response.");
        } finally {
    ...
    /**
     * Return a list of {@link Earthquake} objects by parsing out
     * information from the given json string
     */
    private static List<Earthquake> extractEarthquakesFromJson(String earthquakeListJson) {
        // If the JSON string is empty or null return early
        if (TextUtils.isEmpty(earthquakeListJson)) {
            return null;
        }
        ...
        try {
            ...
            JSONObject jsonObject = new JSONObject(earthquakeListJson);

            JSONArray quakeArray = jsonObject.getJSONArray("features");

            for (int i = 0; i < quakeArray.length(); i++) {
                JSONObject earthquakeObject = quakeArray.getJSONObject(i);
